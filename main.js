...
function onBot() {
  ...
  global.loading(`${crayon(`LOADED`)} ${chalk.blueBright(config.name)} success`, "COMMAND");
...
if (errorMessages.length > 0) {
    ...
    errorMessages.forEach(({ command, error }) => {
        ...
        console.log(`${command}: ${error}`);
                    ...
                    global.loading(`${crayon(`LOADED`)} ${chalk.blueBright(config.name)} success`, "EVENT");

                } 

          catch (err) {

global.loading.err(`${chalk.hex("#ff0000")('ERROR!')} ${chalk.blueBright(ev)} failed with error: ${err.message}`+`\n`, "EVENT");
        ...
        global.loading(`${crayon(`[ SUCCESS ]`)} Loaded ${chalk.blueBright(`${global.client.commands.size}`)} commands and ${chalk.blueBright(`${global.client.events.size}`)} events successfully`, "LOADED");

        global.loading(`${crayon(`[ TIMESTART ]`)} Launch time: ${((Date.now() - global.client.timeStart) / 1000).toFixed()}s`, "LOADED");
...
(async () => {
    ...
    try {
        ...
        global.loading(`${crayon(`[ CONNECT ]`)} Connected to JSON database successfully!`, "DATABASE");

        onBot();

    } catch (error) {

        global.loading.err(`${crayon(`[ CONNECT ]`)} Cannot connect to the JSON database.`, "DATABASE");
